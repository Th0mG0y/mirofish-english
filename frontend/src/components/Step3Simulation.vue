...
const isCreatingDeliberation = ref(false)
...
// Format simulated elapsed time (computed from round number and minutes per round)
...
// Simulated elapsed time for the Reddit platform
...
// Reset all state (used to restart the simulation)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
...
const doStartSimulation = async () => {
  ...
  // Reset all state first to ensure no influence from the previous simulation
  resetAllState()
...
const startDetailPolling = () => {
  detailTimer = setInterval(fetchRunStatusDetail, 10000)
}
...
// Track the last round for each platform to detect changes and output logs
...
const checkPlatformsCompleted = (data) => {
  // If there is no platform data, return false
  ...
  // If at least one platform has completed, check whether all enabled platforms have completed
  // Determine whether a platform is enabled via actions_count (if count > 0 or running was true)
...
const handleDeliberation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`Deliberation session created: ${sessionId}`)
      router.push({ name: 'Deliberation', params: { sessionId } })
    } else {
      addLog(`Failed to create deliberation: ${res.error || 'Unknown error'}`)
      isCreatingDeliberation.value = false
    }
  } catch (err) {
    addLog(`Exception creating deliberation: ${err.message}`)
    isCreatingDeliberation.value = false
  }
}
