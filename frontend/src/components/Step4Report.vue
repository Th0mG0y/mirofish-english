            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generated, but the whole section may not be done yet) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll position to keep the button in the same position
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract statistics – match "Related Predicted Facts: X" format
    const factMatch = text.match(/Related Predicted Facts:\s*(\d+)/)
    ...
    // Extract sub-queries – full extraction, no count limit
    const subQSection = text.match(/### Sub-Questions for Analysis\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract current valid facts – full extraction, no count limit
    ...
    // Extract involved entities – full extraction, no count limit
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Parse selection reason for each individual
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g. 1. **Alumni_345（index=1）**：as a Wuda alumnus...
        ...
        // e.g. - Select Agent_601 (index 0): as representative of the parent group...
        if (!headerMatch) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Extract title (e.g. "Student", "Education Professional")
      ...
      if (qMatch) {
        ...
        // Split questions by numbered index
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // If the first question starts with "1.", handle specially
      ...
      // Extract answer – separated into Twitter and Reddit
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer; copy as default only when not a placeholder
          if (interview.redditAnswer && interview.redditAnswer !== '(No response received on this platform)') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '(No response received on this platform)') {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // No platform markers (very old format) – use entire text as the answer
      ...
      if (quotesMatch) {
        ...
        // Fallback: match > "text" or > \u201Ctext\u201D (Chinese quotation marks)
        if (!quoteMatches) {
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Maintain independent platform tab state for each question-answer pair
    ...
    // Check whether text is a platform placeholder
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(No response received on this platform)' || t === '[No response]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question X:" or "Question X：" (new backend format)
      ...
      // Extract each part by numbering
      const parts = []
    ...
    // Get the answer corresponding to a given question
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split failed: return full answer for the first question, empty for others
      return qIdx === 0 ? answer : ''
    }

    // Check whether a given question has dual-platform answers (filtering out placeholder text)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real answers (not placeholder text) and the content differs
...
const renderMarkdown = (content) => {
  ...
  // Remove the leading second-level heading (## xxx) since the section title is already displayed externally
  ...
  // Clean up <p><br> immediately before block elements (caused by extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean up consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean up <br> before paragraph opening tag that follows a block element
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-consecutive ordered lists: when single-item <ol> is separated by paragraph content, keep count incrementing
        ...
        newLogs.forEach(log => {
          ...
          if (log.action === 'section_complete') {
            if (log.details?.content) {
              ...
              // Automatically expand the newly generated section
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after "Final Answer:" (alternate casing/format)
  const altFinalMatch = response.match(/Final\s*Answer[:：]\s*\n*([\s\S]*)$/i)
  if (altFinalMatch) {
    return altFinalMatch[1].trim()
  }
  
  // If starting with ## or # or >, it may be direct markdown content
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // If content is long and contains markdown, try removing the thought process before returning
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove the thought process starting with "Thought:"
