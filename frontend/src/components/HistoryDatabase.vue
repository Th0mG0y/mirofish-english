...
// Card layout config – adjusted to a wider ratio
...
const containerStyle = computed(() => {
  ...
  // Compute actual required height: rows × card height + (rows-1) × gap + small bottom padding
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Count cards in the current row so each row is centered
...
// Get progress style class based on round count
...
// Format round count display (current/total rounds)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update the pending target state (always record the latest target regardless of animation state)
        pendingState = shouldExpand

        // Clear the previous debounce timer (new scroll intent overrides old one)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // If animating, only record state and handle it after animation ends
        if (isAnimating) return

        // If target state matches current state, nothing to do
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }

        // Use debounce delay to prevent rapid flickering
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if the pending state still needs to execute (may have been overridden by a later scroll)
          ...
          // Release lock after animation completes and check for any pending state changes
          setTimeout(() => {
            isAnimating = false

            // After animation ends, check for new pending state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay a little before executing to avoid switching too quickly
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin: shrink the bottom of the viewport inward so more scrolling is required to trigger expansion
...
// Watch route changes and reload data when returning to home page
...
onMounted(async () => {
  // Wait for DOM to finish rendering before loading data
  await nextTick()
  await loadHistory()

  // Initialise the observer after DOM has rendered
  setTimeout(() => {
    initObserver()
  }, 100)
})

// If keep-alive is used, reload data when component is activated
onActivated(() => {
  loadHistory()
})
...
/* Create a fixed-spacing square grid using CSS background patterns */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Anchor from top-left so that height changes only expand downward without affecting existing grid positions */
...
.cards-container {
  ...
  /* min-height is dynamically computed by JS based on card count */
}
...
/* Low-saturation colour scheme – Morandi palette */
...
/* Bottom round count progress display */
...
/* ===== History playback details modal styles ===== */
.modal-overlay {
  position: fixed;
  top: 0;
  left: 0;
