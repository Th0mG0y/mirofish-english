    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">Simulation Instance Initialisation</span>
          </div>
          <div class="step-status">
            <span v-if="phase > 0" class="badge success">Completed</span>
            <span v-else class="badge processing">Initialising</span>
          </div>
        </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            The LLM intelligently sets world time flow rate, recommendation algorithms, active time periods, posting frequency, event triggers, and other parameters for each individual based on simulation requirements and reality seeds
          </p>
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              ...
              <div class="time-periods">
                ...
                <div class="period-item">
                  <span class="period-label">Work Period</span>
                ...
                <div class="period-item">
                  <span class="period-label">Off-Peak Period</span>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Based on the narrative direction, automatically generates initial activation events and trending topics to guide the initial state of the simulated world
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <p class="description">The simulation environment is ready. You can now start running the simulation.</p>

          <!-- Simulation round count config – only shown after config is generated and rounds are calculated -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Round Count</span>
                <span class="section-desc">MiroFish automatically plans to simulate <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> real-world hours; each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of real-world time passing</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>With 100 agents: estimated time ~{{ Math.round(customMaxRounds * 0.6) }} minutes</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        With 100 agents: estimated time {{ Math.round(autoGeneratedRounds * 0.6) }} minutes
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For a first run, it is strongly recommended to switch to ‘Custom Mode’ to reduce the number of rounds for a quick preview and to lower the risk of errors ➝</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              <span class="info-label">Displayed Age</span>
              <span class="info-value">{{ selectedProfile.age || '-' }} years old</span>
            </div>
            <div class="info-item">
              <span class="info-label">Displayed Gender</span>
            ...
            <div class="info-item">
              <span class="info-label">Displayed MBTI</span>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              <div class="dimension-card">
                ...
                <span class="dim-desc">Complete behavioural trajectory in this event</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">Behavioural Pattern Profile</span>
                <span class="dim-desc">Experience summary and preferred behavioural style</span>
              </div>
              <div class="dimension-card">
...
// State
const phase = ref(0) // 0: initialising, 1: generating personas, 2: generating config, 3: complete
...
// Simulation round count config
const useCustomRounds = ref(false) // Use auto-configured round count by default
...
watch(currentStage, (newStage) => {
  if (newStage === 'generating_profiles') {
    ...
    phase.value = 2
    // Entering config generation stage, start polling for config
    if (!configTimer) {
...
// Calculate auto-generated round count from config (do not use hardcoded defaults)
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // Return null when config has not been generated
  }
  ...
  // Ensure max rounds is not less than 40 (recommended value) to avoid slider range issues
...
// Compute total associated topic count across all personas
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    // User-defined round count, pass max_rounds parameter
    params.maxRounds = customMaxRounds.value
    addLog(`Starting simulation with custom round count: ${customMaxRounds.value} rounds`)
  } else {
    // User chose to keep auto-generated round count, do not pass max_rounds parameter
    addLog(`Starting simulation with auto-configured round count: ${autoGeneratedRounds.value} rounds`)
  }
...
const startPrepareSimulation = async () => {
  ...
  // Mark first step complete, begin second step
  phase.value = 1
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Existing completed preparation detected, loading directly')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        // If all personas have been generated
...
const loadPreparedData = async () => {
  ...
  // Fetch config (using real-time endpoint)
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog('Config generating, starting to poll and wait...')
        startConfigPolling()
      }
    }
  } catch (err) {
...
onMounted(() => {
  ...
  if (props.simulationId) {
    addLog('Step2 Environment Setup Initialising')
...
/* Simulation round count config styles */
