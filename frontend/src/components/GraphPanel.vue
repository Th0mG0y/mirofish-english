  ...
  <div class="graph-panel">
    <div class="panel-header">
      ...
      <div class="header-tools">
        <button v-if="graphData" class="tool-btn" @click="showEdgeLabels = !showEdgeLabels" :class="{ active: showEdgeLabels }" title="Toggle Edge Labels">
...
const renderGraph = () => {
  ...
  // Record which edge index we are at for each pair of nodes
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    // Determine whether the edge direction matches the normalised direction (source UUID < target UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid

    // Calculate curvature: spread multiple edges apart, single edge is straight
    let curvature = 0
    if (totalCount > 1) {
      // Distribute curvature evenly to ensure clear separation
      // Curvature range increases with edge count: more edges, wider range
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2

      // If the edge direction is opposite to the normalised direction, flip the curvature
      // This ensures all edges are distributed in the same reference frame without overlapping
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Dynamically adjust distance based on the number of edges between this pair of nodes
    ...
    // Add gravity toward centre so isolated node clusters are pulled to the centre area
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw an arc that leaves the node and returns
      const loopRadius = 30
    ...
    // Calculate curve control point – dynamically adjusted by edge count and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Offset perpendicular to the connection direction, scaled by distance to keep curves clearly visible
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Only record position, do not restart simulation (distinguish click vs drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        ...
        if (!d._isDragging && distance > 3) {
          // First detection of real drag, now restart simulation
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only wind down simulation if an actual drag occurred
        if (d._isDragging) {
