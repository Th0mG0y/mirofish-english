...
// Only retries on genuine network/timeout errors — never on backend API errors (4xx/5xx)
// to avoid silent duplicate LLM calls
export const requestWithRetry = async (requestFn, maxRetries = 3, delay = 1000) => {
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      const isServerError = error.response && error.response.status >= 400

      // Don't retry backend errors — only retry network/timeout issues
      if (isServerError || i === maxRetries - 1) throw error
