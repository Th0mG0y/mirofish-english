...
# Adding this prefix can avoid Agent calling tools, replying directly with text
INTERVIEW_PROMPT_PREFIX = "Combining your persona, all past memories and actions, reply to me directly with text without calling any tools: "


def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize interview question by adding a prefix to prevent Agent from calling tools
...
def create_simulation():
    ...
    Note: Parameters such as max_rounds are intelligently generated by LLM, no manual setting needed
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    """
    Check if the simulation has been fully prepared.
    ...
    Note: Run scripts (run_*.py) remain in backend/scripts/ directory, no longer copied to the simulation directory.
    ...
    try:
        ...
        # The following statuses all indicate preparation is complete:
        ...
        # - completed: Run complete, preparation was completed earlier
...
def prepare_simulation():
    ...
    Features:
    - Automatically detects completed preparation work, avoids duplicate generation
    - If preparation is already complete, returns existing results directly
    ...
    try:
        ...
        try:
            ...
            # Failure does not affect subsequent process, background task will re-fetch
...
def get_prepare_status():
    ...
    2. Check if preparation is already complete via simulation_id
            ...
            if simulation_id:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "not_started",
                        "progress": 0,
                        "message": "Preparation not yet started, please call /api/simulation/prepare to start",
                        "already_prepared": False
                    }
                })
            return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Iterates through the reports directory, finds reports matching the simulation_id,
    returns the latest one (sorted by created_at) if there are multiple
    ...
    try:
        ...
        # Sort by creation time in descending order, return the latest
...
def get_simulation_history():
    ...
    try:
        ...
        # Enhance simulation data, only reading from Simulation files
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use user-set total_rounds, or use recommended rounds if not set
            ...
            # Get file list for associated project (up to 3)
...
def get_simulation_profiles(simulation_id: str):
    """
    Get Agent Profile for a simulation
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Real-time retrieval of Agent Profile for a simulation (for real-time viewing during generation)

    Differences from the /profiles interface:
    - Reads file directly, does not go through SimulationManager
    - Suitable for real-time viewing during generation
    - Returns additional metadata (such as file modification time, whether generating, etc.)
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"Failed to read profiles file (may be currently being written): {e}")
                profiles = []
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Real-time retrieval of simulation configuration (for real-time viewing during generation)

    Differences from the /config interface:
    - Reads file directly, does not go through SimulationManager
    - Suitable for real-time viewing during generation
    - Returns additional metadata (such as file modification time, whether generating, etc.)
    - Can return partial information even if configuration is not yet fully generated
    ...
    try:
        ...
        if file_exists:
            ...
            try:
                ...
                logger.warning(f"Failed to read config file (may be currently being written): {e}")
                config = None
...
def get_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": "Simulation configuration not found, please call /prepare interface first"
            }), 404
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return jsonify({
                "success": False,
                "error": "Configuration file not found, please call /prepare interface first"
            }), 404
...
# ============== Profile Generation Interface (standalone use) ==============

@simulation_bp.route('/generate-profiles', methods=['POST'])
def generate_profiles():
    """
    Generate OASIS Agent Profile directly from graph (without creating a simulation)
...
def start_simulation():
    ...
    About the force parameter:
        - When enabled, if simulation is running or completed, it stops and clears run logs first
        - Cleared content includes: run_state.json, actions.jsonl, simulation.log, etc.
        - Does not clear configuration files (simulation_config.json) and profile files
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": "Simulation is running, please call /stop to stop first, or use force=true to force restart"
                            }), 400
                ...
                # Process not running or already ended, reset status to ready
...
def get_run_status_detail(simulation_id: str):
    ...
    Used for displaying real-time activity on the frontend.
    ...
    try:
        ...
        # recent_actions only shows the content of the latest round across both platforms
...
def get_simulation_timeline(simulation_id: str):
    ...
    Used for displaying progress bars and timeline views on the frontend.
...
def get_agent_stats(simulation_id: str):
    ...
    Used for displaying Agent activity rankings and action distribution on the frontend.
    """
    try:
...
def interview_agent():
    ...
    (entered waiting-for-command mode after completing the simulation loop).
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // Required, list of interviews
                ...
                {
                    "agent_id": 1,
                    "prompt": "What is your opinion on B?"  // If platform not specified, the default value is used
                }
            ],
            "platform": "reddit",              // Optional, default platform (overridden by each item's platform)
                                               // If not specified: dual-platform simulation interviews each Agent on both platforms simultaneously
    ...
    try:
        ...
        # Optimize the prompt for each interview item, add prefix to prevent Agent from calling tools
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",          // Optional, platform type (reddit/twitter)
                                           // If not specified, returns all history for both platforms
...
def close_simulation_env():
    ...
    Note: This is different from the /stop endpoint. /stop forcibly terminates the process,
