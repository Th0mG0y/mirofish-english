...
class OasisProfileGenerator:
    ...
    Converts entities from the Zep graph into Agent Profiles required for OASIS simulation
    ...
    # Individual type entities (requires generating specific personas)
    INDIVIDUAL_ENTITY_TYPES = [
        "student", "alumni", "professor", "person", "publicfigure", 
        "expert", "faculty", "official", "journalist", "activist"
    ]
    
    # Group/institutional type entities (requires generating representative personas)
    GROUP_ENTITY_TYPES = [
    ...
    def __init__(
        ...
        self.llm_provider = ProviderFactory.create_main_provider(
            api_key=api_key,
            base_url=base_url,
            model=model_name,
        )
    ...
    def _generate_profile_with_llm(
        ...
        Differentiates based on entity type:
        - Individual entities: generate a specific character profile
        - Group/institutional entities: generate a representative account profile
        """
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        # 1. First try to fix truncated content
        ...
        if json_match:
            ...
            # Find all string values and replace their newlines
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        ...
        base_prompt = "You are an expert in generating social media user profiles. Generate detailed, realistic personas for public opinion simulation, reproducing real-world situations as accurately as possible. You must return valid JSON format; all string values must not contain unescaped newlines. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        """Build a detailed persona prompt for individual entities"""

        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "None"
        context_str = context[:3000] if context else "No additional context"

        return f"""Generate a detailed social media user persona for the entity, reproducing real-world situations as accurately as possible.
...
2. persona: Detailed persona description (2000 words of plain text), including:
   ...
   - Personal memory (important part of the persona: describe this individual's connection to events and their past actions and reactions in those events)
...
6. country: Country (in English, e.g. "China")
...
- persona must be a coherent passage of text
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        """Build a detailed persona prompt for group/institutional entities"""

        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "None"
        context_str = context[:3000] if context else "No additional context"

        return f"""Generate a detailed social media account profile for an institution/group entity, reproducing real-world situations as accurately as possible.
...
2. persona: Detailed account profile description (2000 words of plain text), including:
   - Basic institutional information (official name, nature of institution, founding background, primary functions)
   ...
   - Stance and attitude (official position on key topics, how to handle controversies)
   - Special notes (profile of represented group, operational habits)
   - Institutional memory (important part of the persona: describe this institution's connection to events and its past actions and reactions in those events)
3. age: Fixed value of 30 (virtual age for institutional accounts)
4. gender: Fixed value of "other" (institutional accounts use "other" to indicate non-personal)
...
- persona must be a coherent passage of text; do not use newlines
- Use English throughout (except gender must be the English string "other")
...
- Institutional account speech must match its identity positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            realtime_output_path: File path for real-time writing (if provided, written after each generation)
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                # Print generated persona to console and log in real time
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        """Print generated persona to console in real time (full content, no truncation)"""
        separator = "-" * 70
    ...
    def save_profiles(
        ...
        """
        Save profiles to file (selects the correct format based on platform)
    ...
    def _normalize_gender(self, gender: Optional[str]) -> str:
        ...
        # Normalize gender values to standard English strings
        gender_map = {
            "male": "male",
            "female": "female",
            "other": "other",
