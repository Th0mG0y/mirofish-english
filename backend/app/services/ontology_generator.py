...
ONTOLOGY_SYSTEM_PROMPT = """You are a professional knowledge graph ontology design expert. Your task is to analyze given text content and simulation requirements, and design entity types and relationship types suitable for **research-focused analytical simulation and prediction**.

**Important: You must output valid JSON format data, do not output anything else.**

## Core Task Background

We are building a **research-focused analytical simulation and prediction system**. In this system:
- Each entity is a stakeholder, analyst, institution, or participant that can act, interact, and influence outcomes
- Entities influence each other through communication, decisions, actions, and reactions
- We need to simulate the reactions of various parties and the pathways of information propagation and decision-making

Therefore, **entities must be real-world actors — stakeholders, analysts, institutions, and participants**:
...
- Representatives of specific groups (e.g., alumni associations, advocacy groups, industry coalitions, etc.)
...
```json
{
    ...
    "analysis_summary": "Brief analysis description of the text content (in English)"
}
```
...
A. **Fallback types (must include, placed as the last 2 in the list)**:
   - `Person`: Fallback type for any natural person. When a person does not fit any other more specific person type, they are classified here.
   - `Organization`: Fallback type for any organization or institution. When an organization does not fit any other more specific organization type, it is classified here.
...
B. **Specific types (8 types, designed based on text content)**:
   ...
   - Example: If the text involves a commercial event, you can have `Company`, `CEO`, `Employee`

**Why fallback types are needed**:
- Various characters will appear in the text, such as "primary and secondary school teachers", "passersby", "some netizen"
...
- Person: Any natural person (use when they do not fit the above specific types)
...
- Organization: Any organization or institution (use when they do not fit the above specific types)
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        # If text exceeds 50,000 characters, truncate (only affects content passed to LLM, does not affect graph building)
...
"""
        ...
        message += """
Based on the above content, please design entity types and relationship types suitable for research-focused analytical simulation.
...
4. All entity types must be real-world actors — stakeholders, analysts, institutions, participants — they cannot be abstract concepts
...
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if fallbacks_to_add:
            ...
            # If adding would exceed 10 types, need to remove some existing ones
    ...
    def generate_python_code(self, ontology: Dict[str, Any]) -> str:
        ...
        code_lines = [
            ...
            'Auto-generated by MiroFish for analytical simulation',
            '"""',
            '',
            ...
            '',
            '',
            '# Base model types for ontology definitions',
