...
class SimulationRunState:
    ...
    # Per-platform independent round and simulated time (for dual-platform parallel display)
    ...
    # Process ID (for stopping the simulation)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            # Per-platform independent round and time
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Maximum number of simulation rounds (optional, to cap overly long simulations)
        enable_graph_memory_update: bool = False,  # Whether to update activities to the Zep graph
        ...
        Args:
            ...
            platform: Target platform (twitter/reddit/parallel)
            max_rounds: Maximum number of simulation rounds (optional, to cap overly long simulations)
        ...
        if not os.path.exists(config_path):
            raise ValueError(f"Simulation configuration not found; please call the /prepare endpoint first")
        ...
        # Truncate to max_rounds if specified
        ...
        # Create updater if graph memory update is enabled
        ...
        # Determine which script to run (scripts are located in backend/scripts/)
        if platform == "twitter":
        ...
        try:
            ...
            cmd = [
                ...
                script_path,
                "--config", config_path,  # Full configuration file path
            ]
            ...
            # Create the main log file to prevent stdout/stderr pipe buffer overflow from blocking the process
            ...
            # Set the working directory to the simulation directory (databases etc. are generated there)
            # start_new_session=True creates a new process group so os.killpg can terminate all child processes
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create a new process group for clean shutdown
            )

            # Save file handles for later cleanup
            cls._stdout_files[simulation_id] = main_log_file
            cls._stderr_files[simulation_id] = None  # No separate stderr file needed
    ...
    # News injection tracking
    _news_injection_services: Dict[str, Any] = {}
    _news_last_injected: Dict[str, float] = {}
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        # Initialize news injection if configured
        news_interval = Config.MIROFISH_NEWS_INJECTION_INTERVAL
        if news_interval > 0:
            try:
                from .news_injection_service import NewsInjectionService
                cls._news_injection_services[simulation_id] = NewsInjectionService()
                cls._news_last_injected[simulation_id] = time.time()
                logger.info(f"News injection enabled: interval={news_interval}s, simulation={simulation_id}")
            except Exception as e:
                logger.warning(f"Failed to initialize news injection: {e}")
                news_interval = 0
        ...
        try:
            while process.poll() is None:  # Process is still running
                ...
                # Periodic news injection
                if news_interval > 0 and simulation_id in cls._news_injection_services:
                    now = time.time()
                    last = cls._news_last_injected.get(simulation_id, 0)
                    if now - last >= news_interval:
                        try:
                            news_service = cls._news_injection_services[simulation_id]
                            # Load simulation requirement from config
                            config_path = os.path.join(sim_dir, "simulation_config.json")
                            if os.path.exists(config_path):
                                ...
                                if requirement:
                                    news_items = news_service.generate_news_seeds(requirement=requirement)
                                    if news_items:
                                        news_service.inject_into_simulation(simulation_id, news_items)
                                        logger.info(f"Injected {len(news_items)} news items into simulation {simulation_id}")
                            cls._news_last_injected[simulation_id] = now
                        except Exception as e:
                            logger.warning(f"News injection failed (non-fatal): {e}")
                            cls._news_last_injected[simulation_id] = now
            ...
            # Do a final log read after the process exits
            if os.path.exists(twitter_actions_log):
            ...
            if exit_code == 0:
                ...
                state.runner_status = RunnerStatus.FAILED
                # Read error info from the main log file
                main_log_path = os.path.join(sim_dir, "simulation.log")
                error_info = ""
                try:
            ...
            # Clean up news injection resources
            cls._news_injection_services.pop(simulation_id, None)
            cls._news_last_injected.pop(simulation_id, None)
                    ...
                    if line:
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Detect simulation_end events and mark platform as completed
                                if event_type == "simulation_end":
                                    ...
                                    # If two platforms are running, both must be completed
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prefer the platform field in the record, fall back to default platform
    ...
    def get_all_actions(
        ...
        # If per-platform files don't exist, try reading the old single-file format
        if not actions:
            actions_log = os.path.join(sim_dir, "actions.jsonl")
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # Old format files should have the platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation run logs (for forcing a fresh restart of the simulation)
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check whether there is anything to clean (avoid printing useless logs for empty processes)
        ...
        # First stop all graph memory updaters (stop_all prints its own logs internally)
        try:
    ...
    def register_cleanup(cls):
        ...
        Called when the Flask application starts, to ensure all child processes are
        terminated when the server shuts down.
        """
        ...
        # In Flask debug mode, only register cleanup in the reloader subprocess (the process that actually runs the app)
        # WERKZEUG_RUN_MAIN=true indicates the reloader subprocess
        # If not in debug mode, this env var is absent and we still need to register
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None

        # In debug mode, only register in the reloader subprocess; always register in non-debug mode
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            # Only print log when there are processes to clean up
        ...
        try:
            # SIGTERM: default signal for the kill command
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True if the environment is alive, False if it has closed
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: In dual-platform simulation, interview both platforms simultaneously and return merged result
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by each interview item's platform)
                - "twitter": Interview Twitter platform only by default
                - "reddit": Interview Reddit platform only by default
    ...
    def interview_all_agents(
        ...
        Args:
            simulation_id: Simulation ID
            prompt: Interview question (same question used for all Agents)
            platform: Specify platform (optional)
                - "twitter": Interview Twitter platform only
                - "reddit": Interview Reddit platform only
                - None: In dual-platform simulation, each Agent is interviewed on both platforms simultaneously
    ...
    def close_simulation_env(
        ...
        try:
            ...
            # Timeout may be because the environment is in the process of closing
            return {
                "success": True,
    ...
    def get_interview_history(
        ...
        # If multiple platforms were queried, cap the total
