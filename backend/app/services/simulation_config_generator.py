...
Uses a step-by-step generation strategy to avoid failure from generating excessively long content at once:
...
# Timezone presets for different regions
TIMEZONE_PRESETS = {
    "us_eastern": {
...
def _get_timezone_config():
    """Get the timezone configuration based on MIROFISH_TIMEZONE setting"""
    from ..config import Config
    tz = getattr(Config, 'MIROFISH_TIMEZONE', 'us_eastern')
    return TIMEZONE_PRESETS.get(tz, TIMEZONE_PRESETS['us_eastern'])

# Default timezone config (resolved at import time for backward compat)
DEFAULT_TIMEZONE_CONFIG = TIMEZONE_PRESETS['us_eastern']
...
class AgentActivityConfig:
    ...
    # Response speed (reaction delay to hot events, in simulation minutes)
...
class TimeSimulationConfig:
    """Time simulation configuration (configurable timezone, defaults to US Eastern)"""
    ...
    # Peak hours (evening 18-22, most active period)
    peak_hours: List[int] = field(default_factory=lambda: [18, 19, 20, 21, 22])
    peak_activity_multiplier: float = 1.5

    # Off-peak hours (1-6 AM, almost no activity)
...
class PlatformConfig:
    ...
    # Viral spread threshold (number of interactions needed to trigger spread)
    viral_threshold: int = 10

    # Echo chamber effect strength (clustering degree of similar opinions)
...
class SimulationConfigGenerator:
    ...
    and automatically generates optimal simulation parameter configuration
    ...
    def __init__(
        ...
        self.provider = ProviderFactory.create_main_provider(
            api_key=api_key,
            base_url=base_url,
            model=model_name,
        )
    ...
    def generate_config(
        ...
        """
        logger.info(f"Starting intelligent simulation configuration generation: simulation_id={simulation_id}, entity_count={len(entities)}")
    ...
    def _call_llm_with_retry(self, prompt: str, system_prompt: str) -> Dict[str, Any]:
        """LLM call with retry mechanism, includes JSON repair logic"""
        import re
...
- Activity follows typical daily patterns for the target population
- 1-6 AM: almost no activity (activity multiplier 0.05)
- 7-9 AM: gradually becoming active (activity multiplier 0.4)
- 10-17 work hours: medium activity (activity multiplier 0.7)
- 18-22 PM evening: peak period (activity multiplier 1.5)
- After 23 PM: activity decreases (activity multiplier 0.5)
- General pattern: low activity at dawn, increasing in morning, medium during work hours, peak in evening
- **Important**: The following example values are for reference only; adjust specific time periods based on event nature and participant characteristics
  - Example: student groups may peak at 21-23 PM; media active all day; official institutions only during work hours
  - Example: breaking hot topics may lead to late-night discussions, off_peak_hours can be shortened
...
- peak_hours (int array): Peak period hours, adjusted based on event participants
- off_peak_hours (int array): Off-peak hours, usually late night/dawn
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a research simulation configuration expert. Return pure JSON format, time configuration should match typical daily activity patterns for the target population."
    ...
    def _get_default_time_config(self, num_entities: int) -> Dict[str, Any]:
        """Get default time configuration based on configured timezone"""
        tz_config = _get_timezone_config()
        return {
            "total_simulation_hours": 72,
            "minutes_per_round": 60,  # 1 hour per round, speeds up time flow
            "agents_per_hour_min": max(1, num_entities // 15),
            "agents_per_hour_max": max(5, num_entities // 5),
            "peak_hours": tz_config["peak_hours"],
            "off_peak_hours": tz_config["dead_hours"],
            "morning_hours": tz_config["morning_hours"],
            "work_hours": tz_config["work_hours"],
            "reasoning": "Using default timezone configuration (1 hour per round)"
        }

    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        """Parse time configuration result, validate agents_per_hour values don't exceed total agent count"""
        # Get raw values
        agents_per_hour_min = result.get("agents_per_hour_min", max(1, num_entities // 15))
        agents_per_hour_max = result.get("agents_per_hour_max", max(5, num_entities // 5))
        
        # Validate and correct: ensure values don't exceed total agent count
...
{{
    ...
    "reasoning": "<brief explanation>"
        ...
        system_prompt = "You are a research simulation analysis expert. Return pure JSON format. Note that poster_type must exactly match available entity types."
    ...
    def _assign_initial_post_agents(
        ...
        # Type mapping table (handles different formats that LLM might output)
        type_aliases = {
        ...
        for post in event_config.initial_posts:
            ...
            if matched_agent_id is None:
                ...
                if agent_configs:
                    # Sort by influence, select the one with highest influence
...
- **Time follows typical daily patterns**: 1-6 AM almost no activity, 18-22 PM evening most active
- **Official institutions** (University/GovernmentAgency): low activity (0.1-0.3), work hours (10-17) activity, slow response (60-240 min), high influence (2.5-3.0)
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<list of active hours based on typical daily patterns>],
        ...
        system_prompt = "You are a research simulation behavior analysis expert. Return pure JSON, configuration should follow typical daily activity patterns for the target population."
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate single Agent configuration using rules (configurable timezone)"""
        ...
        if entity_type in ["university", "governmentagency", "ngo"]:
            ...
            # Alumni: mainly evening activity
            return {
                "activity_level": 0.6,
                "posts_per_hour": 0.4,
