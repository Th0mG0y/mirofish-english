...
Reads parameters from a configuration file to run the simulation, fully automated
...
class TwitterSimulationRunner:
    ...
    def _create_model(self):
        ...
        if not llm_model:
            llm_model = self.config.get("llm_model", "gpt-5.4-mini")
    ...
    def _get_active_agents_for_round(
        ...
        # Calculate activation probability for each Agent based on its config
        candidates = []
...
async def main():
    ...
    # Initialize logging configuration (using fixed filenames, clean up old logs)
...
def setup_signal_handlers():
    ...
    Gives the program a chance to clean up resources gracefully (close database, environment, etc.)
    """
