...
# This is to fix the issue where the OASIS third-party library reads files without specifying encoding
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # This affects all open() calls without a specified encoding
    os.environ.setdefault('PYTHONUTF8', '1')
    os.environ.setdefault('PYTHONIOENCODING', 'utf-8')

    # Reconfigure standard output streams to UTF-8 (fixes garbled console output)
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        """
        Wraps the open() function to default to UTF-8 encoding for text mode
        This fixes the issue where third-party libraries (such as OASIS) read files without specifying encoding
        """
        # Only set the default encoding for text mode (non-binary) without a specified encoding
...
# Load the .env file from the project root directory (contains LLM_API_KEY and other configuration)
...
# Add the filter immediately at module load time, to take effect before camel code runs
...
def disable_oasis_logging():
    ...
    OASIS logs are too verbose (recording each agent's observations and actions); we use our own action_logger
    """
...
class ParallelIPCHandler:
    ...
    Manages the environments of both platforms and handles Interview commands
    """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None/unspecified: Interview both platforms simultaneously and return merged result
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Items that need to interview both platforms simultaneously
        ...
        # Split both_platforms_interviews across both platforms
...
# Non-core action types to filter out (these actions have lower analytical value)
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Maximum rowid from the last read (using rowid instead of created_at because created_at format differs between platforms)
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, keeping only key fields (retain full content, no truncation)
            simplified_args = {}
...
def _enrich_action_context(
    ...
    Args:
        ...
        action_args: Action parameters (will be modified in place)
        agent_names: agent_id -> agent_name mapping
    """
    try:
        ...
        # Repost: add original post content and author
            ...
            if new_post_id:
                # original_post_id of the repost points to the original post
                cursor.execute("""
        ...
        # Quote post: add original post content, author, and quote comment
            ...
            # Get the quote comment content of the quoted post (quote_content)
            if new_post_id:
                cursor.execute("""
        ...
        # Create comment: add the post being commented on
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    - General configuration: LLM_API_KEY, LLM_BASE_URL, LLM_MODEL_NAME
    - Boost configuration (optional): LLM_BOOST_API_KEY, LLM_BOOST_BASE_URL, LLM_BOOST_MODEL_NAME

    If a boost LLM is configured, different platforms can use different API providers during parallel simulation,
    increasing concurrency capacity.
    ...
    if not llm_model:
        llm_model = config.get("llm_model", "gpt-5.4-mini")
...
async def run_twitter_simulation(
    ...
    # Get the real Agent name mapping from config (using entity_name instead of default Agent_X)
    ...
    total_actions = 0
    last_rowid = 0  # Track the last processed row in the database (using rowid to avoid created_at format differences)
    ...
    for round_num in range(total_rounds):
        ...
        if _shutdown_event and _shutdown_event.is_set():
            if main_logger:
                main_logger.info(f"Exit signal received, stopping simulation at round {round_num + 1}")
            break
        ...
        # Always record round start, regardless of whether there are active agents
        if action_logger:
...
async def run_reddit_simulation(
    ...
    # Reddit uses the boost LLM configuration (if available, otherwise falls back to general)
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close the environment immediately after simulation, do not enter command-waiting mode'
    )
    
    args = parser.parse_args()
    
    # Create the shutdown event at the start of main, so the entire program can respond to exit signals
    ...
    # Initialize logging configuration (disable OASIS logging, clean up old files)
    ...
    # Enter command-waiting mode if enabled
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entering command-waiting mode - environment remains running")
...
def setup_signal_handlers(loop=None):
    """
    Set up signal handlers to ensure correct exit on SIGTERM/SIGINT

    Persistent simulation scenario: do not exit after simulation completes; wait for interview commands.
    When a termination signal is received:
    1. Notify the asyncio loop to exit waiting
    2. Give the program a chance to clean up resources gracefully (close database, environment, etc.)
    3. Then exit
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set the event to notify the asyncio loop to exit (giving it a chance to clean up resources)
            if _shutdown_event:
                _shutdown_event.set()
